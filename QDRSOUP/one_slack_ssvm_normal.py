...
class OneSlackSSVM(BaseSSVM):
    ...
    def _solve_1_slack_qp(self, constraints, n_samples):
        ifStop = False # No regularizing QP
        C = np.float64(self.C) * n_samples  # this is how libsvm/svmstruct do it
        ...
        #print(np.concatenate([-np.array(losses, dtype=np.float64),np.zeros(len(self.w))]))
        #input("Press Enter to continue...")
        if nonzerow==0:
            q = cvxopt.matrix(-np.array(losses, dtype=np.float64))
        else:
            q = cvxopt.matrix(np.concatenate([-np.array(losses, dtype=np.float64),np.zeros(nonzerow)]))
        ...
        if solution['status'] != "optimal":
            ...
            ifStop = True
            return -solution['primal objective'], ifStop
    ...
    def fit(self, X, Y, constraints=None, warm_start=False, initialize=True):
        ...
        try:
            ...
            for iteration in range(self.max_iter):
                ...
                if self.log is not None:
                    self.writeLog("iteration " + str(iteration)+" normal",toconsole = True)
                ...
                objective, ifStop = self._solve_1_slack_qp(constraints,
                                                   n_samples=len(X))
                if ifStop:
                    print("break due to regularizing QP")
                    break
                #if self.log is not None:
