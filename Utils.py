...
class Utils(object):
    ...
    def getWeibull_alpha_lambda(alpha, lamb):
        return lamb*np.random.weibull(alpha,1)[0]
    ...
    def getPreTrain():
        ...
        while True: 
            ...
            if not line: 
                break   
            strings = line.split()
            lists[strings[0]]=strings[1]
        file1.close()
        inpaths = ["AWS copy/AWS1/SI_pre_train/pre_train/", "AWS copy/AWS2/SI_pre_train/pre_train/", "AWS copy/AWS3/SI_pre_train/pre_train/", "AWS copy/AWS4/SI_pre_train/pre_train/"]
        for inPath in inpaths:
            for f in listdir(inPath):
                for file in listdir(inPath+f):
                    if file in lists:
                        print(inPath+f+'/'+file)         
                        from_dir = inPath+f+'/'+file
                        to_dir = "pre_train_final/"+lists[file]+"-"+file
...
class Dijkstra(object): 
    ...
    def dijkstra_batch(graph, srcs, results = None): 
        results = []
        for src in srcs:
            results.append(Dijkstra.dijkstra(graph, src))      
        return results
